...
import { MicModeGateContent } from '@/components/ConsentUnlock'
...
export function MicCheck({ onStart, onCancel, subject, initialAcknowledged }: Props) {
  const [step, setStep] = useState<'gate' | 'check'>(initialAcknowledged ? 'check' : 'gate')
  ...
  const acknowledge = async () => {
    ...
    try {
      ...
      if (session?.user) {
        await sb.from('mic_acknowledgments').upsert(
          { user_id: session.user.id, subject: subject || '' },
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const tick = () => {
        ...
        const step2 = Math.floor(data.length / 7)
        setBars(Array.from({ length: 7 }, (_, i) => data[i * step2] / 255))
