      ...
      <div className="relative overflow-hidden px-4 sm:px-6 pt-6 pb-5">
        ...
        {loading ? (
          <div className="animate-pulse">
            <div className="h-10 w-24 dark:bg-white/[0.07] bg-black/[0.06] rounded-xl mb-2" />
            <div className="h-3.5 w-48 dark:bg-white/[0.04] bg-black/[0.03] rounded-full" />
          </div>
        ) : (
          <>
        ...
        {!loading && totalCount > 0 && (
          <div className="relative mt-5">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 dark:bg-white/[0.05] bg-black/[0.04] border dark:border-white/[0.08] border-black/[0.08] rounded-2xl text-[14px] dark:text-white text-gray-900 placeholder-gray-700 focus:outline-none focus:border-yellow-500/40 focus:dark:bg-white/[0.07] bg-black/[0.06] transition-all"
            />
          </div>
        )}
      </div>
      ...
      <div className="px-4 sm:px-6 pb-6">
            ...
            {[4, 3, 5].map((count, i) => (
              <div key={i}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 h-px dark:bg-white/[0.04] bg-black/[0.03]" />
                  <div className="h-3 w-12 dark:bg-white/[0.04] bg-black/[0.03] rounded-full" />
                </div>
                <div className="dark:bg-white/[0.03] bg-black/[0.025] border dark:border-white/[0.06] border-black/[0.07] rounded-2xl overflow-hidden">
                  {Array.from({ length: count }).map((_, j) => (
                    <div key={j} className={`px-4 py-4 flex gap-3 ${j > 0 ? 'border-t dark:border-white/[0.04] border-black/[0.05]' : ''}`}>
                      <div className="w-[3px] shrink-0 rounded-full dark:bg-white/[0.06] bg-black/[0.05] self-stretch" />
                      <div className="flex-1">
                        <div className="h-4 w-32 dark:bg-white/[0.08] bg-black/[0.06] rounded-full mb-2" />
                        <div className="h-3 w-full dark:bg-white/[0.05] bg-black/[0.04] rounded-full" />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {!loading && totalCount === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center gap-2">
            <div className="w-12 h-12 rounded-2xl dark:bg-white/[0.04] bg-black/[0.03] border dark:border-white/[0.07] border-black/[0.07] flex items-center justify-center mb-2">
            ...
            {filteredSessions.map(s => (
              <div key={s.id}>
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  <span className={`text-[13px] font-semibold shrink-0 truncate max-w-[180px] ${s.name ? 'dark:text-white/80 text-gray-800' : 'text-gray-500'}`}>
                  ...
                  {s.subject && (
                    <span className="text-[11px] text-gray-700 dark:bg-white/[0.04] bg-black/[0.03] border dark:border-white/[0.06] border-black/[0.07] rounded-full px-2 py-[2px] truncate max-w-[100px]">
                      {s.subject}
                    </span>
                  )}
                  <div className="flex-1 h-px dark:bg-white/[0.05] bg-black/[0.04]" />
