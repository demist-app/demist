...
export default function Stats() {
  ...
  return (
    <main className="min-h-dvh bg-[#080810] text-white flex flex-col nav-bottom-pad">
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-5 max-w-xl">
        ...
        {/* Key numbers */}
        <div className="grid grid-cols-2 gap-3 animate-step opacity-0" style={{ animationDelay: '50ms', animationFillMode: 'forwards' }}>
        ...
        {dailySessions.some(d => d.count > 0) && (
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl px-4 py-4 animate-step opacity-0" style={{ animationDelay: '100ms', animationFillMode: 'forwards' }}>
            ...
            <div className="flex items-end gap-1.5 h-[52px]">
              {dailySessions.map((d, i) => {
                const h = Math.max(3, Math.round((d.count / maxDaily) * 44))
        ...
        {weeklyTerms.some(w => w.count > 0) && (
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl px-4 py-4 animate-step opacity-0" style={{ animationDelay: '150ms', animationFillMode: 'forwards' }}>
            ...
            <div className="flex items-end gap-1 h-[52px]">
              {weeklyTerms.map((w, i) => {
                const h = Math.max(3, Math.round((w.count / maxWeekly) * 44))
                const isThis = i === weeklyTerms.length - 1
                return (
                  <div key={i} className="flex flex-col items-center gap-1 flex-1">
                    <div
                      className={`w-full rounded-sm transition-all ${isThis ? 'bg-emerald-500' : 'bg-white/[0.10]'}`}
                      style={{ height: `${h}px` }}
                    />
        ...
        {/* Subjects breakdown */}
        {subjects.length > 0 && (
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl px-4 py-4 animate-step opacity-0" style={{ animationDelay: '200ms', animationFillMode: 'forwards' }}>
                ...
                return (
                  <div key={subject}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[13px] text-white/80 truncate max-w-[70%]">{subject}</span>
